...
void init(ll v, ll b, ll e)
{
    ...
    init(l, b, m);
    init(r, m+1, e);
    tab[v].sum[0] = tab[l].sum[0] + tab[r].sum[0] ;
    tab[v].sum[1] = tab[l].sum[1] + tab[r].sum[1] ;
    tab[v].sum[2] = tab[l].sum[2] + tab[r].sum[2] ;
}
...
ll query(ll v, ll b, ll e, ll i, ll j, ll c=0)
{
    ...
    if(tab[v].prop) {
        tab[l].prop += tab[v].prop ;
        tab[r].prop += tab[v].prop ;

        for(int x=0; x<tab[v].prop%3; x++) {
            ll temp = tab[l].sum[2] ;
            tab[l].sum[2] = tab[l].sum[1] ;
            tab[l].sum[1] = tab[l].sum[0] ;
            tab[l].sum[0] = temp ;
            temp = tab[r].sum[2] ;
            tab[r].sum[2] = tab[r].sum[1] ;
            tab[r].sum[1] = tab[r].sum[0] ;
            tab[r].sum[0] = temp ;
        }
...
void update(ll v, ll b, ll e, ll i, ll j, ll val=0)
{
    ...
    if(b >= i && e <= j)
    {
        tab[v].prop++ ;
        ll temp = tab[v].sum[2] ;
        tab[v].sum[2] = tab[v].sum[1] ;
        tab[v].sum[1] = tab[v].sum[0] ;
        tab[v].sum[0] = temp ;
        return;
    }
